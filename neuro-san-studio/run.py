...
class NeuroSanRunner:
    ...
    def set_environment_variables(self):
        ...
        # Custom SSL Certificate Bundle for Corporate Proxies (ZSclaer etc)
        custom_cert_path = os.path.join(self.root_dir, "custom_cacert.pem")
        if os.path.exists(custom_cert_path):
             print(f"Found custom SSL certificate bundle at: {custom_cert_path}")
             os.environ["SSL_CERT_FILE"] = custom_cert_path
             os.environ["REQUESTS_CA_BUNDLE"] = custom_cert_path
        else:
             print("No custom SSL certificate bundle found (custom_cacert.pem). Using system defaults.")
