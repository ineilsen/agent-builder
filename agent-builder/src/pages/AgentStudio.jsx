...
import { Wrench, Save, Play, Code, Settings, Layers, MessageCircle, Terminal, Sun, Moon, X, Menu, Maximize2, Minimize2, ChevronLeft, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="h-full w-full flex flex-col bg-gray-100 dark:bg-[#0a0b0d] overflow-hidden text-gray-900 dark:text-gray-100 font-sans">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <button
              ...
              className="p-1 rounded-md hover:bg-gray-100 dark:hover:bg-white/10 text-gray-500"
            >
            ...
            <div>
              ...
              <span className="text-[10px] text-gray-500">Neuro SAN Architect</span>
            </div>
          </div>

          <div className="flex items-center space-x-2">
            <div className="px-3 py-1 bg-gray-100 dark:bg-white/5 rounded text-xs text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-white/10">
              {currentNetwork ? (
                <span className="font-mono text-blue-600 dark:text-blue-400">{currentNetwork}.hocon</span>
              ) : (
      ...
      <div className="flex-1 flex overflow-hidden min-h-0 relative">
        ...
        <div className={`transition-all duration-300 ease-in-out border-r border-gray-300 dark:border-white/5 overflow-hidden ${isSidebarOpen ? 'w-[300px]' : 'w-0 border-none'}`}>
          <NetworkSidebar />
        </div>

        {/* COMPONENT TOOLBAR (Old Left Sidebar - Now Minimized/Floating or Integrated) */}
        {/* Let's keep components available but maybe more compact or part of a secondary palette */}
        <div className="w-[60px] shrink-0 border-r border-gray-300 dark:border-white/5 bg-white dark:bg-[#0a0b0d] flex flex-col items-center py-4 gap-4 z-10">
          {/* Component Icons Only */}
          {components.map(comp => (
            <div key={comp.id} className="group relative" title={comp.name}>
              <div className="w-10 h-10 rounded-lg bg-gray-100 dark:bg-white/5 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:bg-blue-50 dark:hover:bg-blue-500/10 hover:text-blue-600 dark:hover:text-blue-400 cursor-grab active:cursor-grabbing transition-colors">
                <comp.icon className="w-5 h-5" />
              </div>
              {/* Tooltip */}
              <div className="absolute left-12 top-2 bg-gray-900 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50">
                {comp.name}
              </div>
            </div>
          ))}

          <div className="h-px w-8 bg-gray-200 dark:bg-white/10 my-2"></div>

          <button className="w-10 h-10 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">
